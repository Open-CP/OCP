...
def gen_cutoff_constraints_upper(cipher, r, obj_layer, best_obj=[]):
    add_cons = []
    for i in range(1, len(best_obj)+1):
        if best_obj[i-1] > 0:
            vars = []
            for j in range(i+1, r+1):
                for cons in cipher.states["STATE"].constraints[j][obj_layer]:
...
def gen_cutoff_constraints_lower(cipher, r, obj_layer, best_obj):
    add_cons = []
    for i in range(1, len(best_obj)+1):
        if best_obj[i-1] > 0:
            vars = []
...
if __name__ == '__main__':
    ...
    for r in range(R, R+1):
        ...
        add_cons += gen_cutoff_constraints_upper(cipher, r, 1, BEST_OBJ)
        add_cons += gen_cutoff_constraints_lower(cipher, r, 1, BEST_OBJ)
