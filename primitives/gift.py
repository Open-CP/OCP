    ...
    def __init__(self, name, version, s_input, s_output, nbr_rounds=None, represent_mode=0):
        ...
        if represent_mode==0:
            for i in range(1,nbr_rounds+1):              
                ...
                self.states["STATE"].AddConstantLayer("C", i, 2, "xor", [True]+[None]*(version-25)+[True]+[None, None, None, True]*5, constant_table)# Constant layer                      
...
class GIFT_block_cipher(Block_cipher):
    def __init__(self, name, version, p_input, k_input, c_output, nbr_rounds=None, represent_mode=0):
        ...
        if represent_mode==0:
            for i in range(1,nbr_rounds+1):    
                # subkeys extraction
                self.states["SUBKEYS"].ExtractionLayer("SK_EX", i, 0, [item for pair in zip(range(160-p_bitsize, 160-3*int(p_bitsize/4)), range(128-int(p_bitsize/4), 128)) for item in pair], self.states["KEY_STATE"].vars[i][0])
                ...
                self.states["STATE"].PermutationLayer("P", i, 1, perm) # Permutation layer
                self.states["STATE"].AddConstantLayer("C", i, 2, "xor", [True]+[None]*(p_bitsize-25)+[True]+[None, None, None, True]*5, constant_table)# Constant layer                      
                if p_bitsize == 64: self.states["STATE"].AddRoundKeyLayer("ARK", i, 3, op.bitwiseXOR, self.states["SUBKEYS"], [0,0,1,1]*16) # Addroundkey layer 
                elif p_bitsize == 128: self.states["STATE"].AddRoundKeyLayer("ARK", i, 3, op.bitwiseXOR, self.states["SUBKEYS"], [0,1,1,0]*32) # Addroundkey layer 
