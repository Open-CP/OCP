    ...
    def __init__(self, name, s_input, s_output, nbr_rounds=None, represent_mode=0):
        ...
        if represent_mode==0:
            for r in range(1, nbr_rounds+1):
                ...
                S.SboxLayer("SB", r, 1, op.AES_Sbox, mask=[0 for i in range(16)]+[1 for i in range(16)]+[0 for i in range(16)]+[1 for i in range(16)]+[0 for i in range(16)]+[1 for i in range(16)]+[1 for i in range(16)]) # Sbox layer 
                ...
                S.SingleOperatorLayer("XOR", r, 3, op.bitwiseXOR, [[i, i+16*(8+2*(r-1))] for i in range(16)] + [[i+16, i] for i in range(16)] + [[i+16*2, i+16*7] for i in range(16)] + [[i+16*3, i+16*2] for i in range(16)] + [[i+16*4, i+16*(8+2*(r-1)+1)] for i in range(16)] + [[i+16*5, i+16*4]  for i in range(16)] + [[i+16*6, i+16*(8+2*nbr_rounds+1)] for i in range(16)] + [[i+16*7, i+16*(8+2*nbr_rounds)] for i in range(16)],[i for i in range(16*8)]) # XOR layer 
            self.rounds_code_if_unrolled()
        
    def rounds_code_if_unrolled(self):
        self.rounds_python_code_if_unrolled = {"STATE": []}
        self.rounds_c_code_if_unrolled = {"STATE": []}
        for r in range(1, self.states['STATE'].nbr_rounds+1):
            self.rounds_python_code_if_unrolled["STATE"].append([r, f"if i == {r-1}:"])
            self.rounds_c_code_if_unrolled["STATE"].append([r, f"if (i == {r-1})"+"{"])

class Rocca_AD_permutation2(Permutation):
    def __init__(self, name, s_input, s_output, nbr_rounds=None, represent_mode=0):
        """
        Initialize the Rocca_AD2 internal permutation
        :param name: Name of the permutation
        :param version: Bit size of the permutation
        :param s_input: Input state
        :param s_output: Output state
        ...
        if represent_mode==0:
            for r in range(1, nbr_rounds+1):
                S.SingleOperatorLayer("AES_r", r, 0, op.AESround, [[i for i in range(16)]] + [[i for i in range(16*2, 16*3)]] + [[i for i in range(16*4, 16*5)]] + [[i for i in range(16*5, 16*6)]], [[i for i in range(nbr_words, nbr_words+16)]] + [[i for i in range(16*2, 16*3)]] + [[i for i in range(nbr_words+16, nbr_words+16*2)]] + [[i for i in range(16*5, 16*6)]]) # XOR layer 
                S.SingleOperatorLayer("XOR", r, 1, op.bitwiseXOR, [[i+7*16, i+16*(8+2*(r-1))] for i in range(16)] + [[i+nbr_words, i+16*7] for i in range(16)] + [[i+16*1, i+16*6] for i in range(16)] + [[i+16*2, i+16*1] for i in range(16)] + [[i+16*3, i+16*(8+2*(r-1)+1)] for i in range(16)] + [[i+nbr_words+16, i+16*3]  for i in range(16)] + [[i+16*5, i+16*4] for i in range(16)] + [[i, i+16*6] for i in range(16)],[i for i in range(16*8)]) # XOR layer 
            self.rounds_code_if_unrolled()
